...
export default function TechnicalFilterModal({ isOpen, type, onClose }: any) {
  ...
  useEffect(() => {
    ...
    const fetchStep1 = async () => {
      ...
      try {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL}/api/products?filters[${field}][$notNull]=true&fields[0]=${field}&pagination[pageSize]=100`;
  ...
  useEffect(() => {
    ...
    const fetchStep2 = async () => {
      ...
      try {
        const field = FIELD_MAP[type];
        const url = `${process.env.NEXT_PUBLIC_API_URL}/api/products?filters[${field}][$eq]=${encodeURIComponent(selections.step1)}&fields[0]=productType&pagination[pageSize]=100`;
